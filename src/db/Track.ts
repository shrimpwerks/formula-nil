...
const _data: { id: string; name: string }[] = [
    {
        id: "560cb995-ea6c-41da-8e33-a5bf4d2d7cb4",
...
export default class Track {
    ...
    static all(): Track[] {
        return _data.map((track) => new Track(track.id, track.name));
    }

    static find(id: string): Track | undefined {
        return this.all().find((track) => track.id === id);
    }
    ...
    generateTrackPoints(
        ...
        const radiusFunction = (angle: number): number => {
            ...
            features.forEach((feature) => {
                const influence = this.getFeatureInfluence(
                    angle,
                    feature.startAngle,
                    feature.endAngle,
                    feature.type,
